...
import { createCrewMember } from '@/lib/operations-actions'
...
export default async function NewCrewMemberPage({
  ...
  // Fetch existing crew from Weekend Crew List
  const crewContacts = await prisma.operationsContact.findMany({
    where: { isCrew: true, archivedAt: null },
    orderBy: { name: 'asc' },
  })

  const createCrew = createCrewMember.bind(null, event.id, `/events/${event.id}/operations`)
    ...
    <main className="min-h-screen bg-black p-8 text-white">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="rounded-2xl border border-[#2A0E10] bg-[#0B0B0B] p-8">
          <h1 className="text-3xl font-bold text-white">Add Crew Member</h1>
          <p className="mt-2 text-[#B7B7B7]">Assign a person from the Weekend Crew List to this event.</p>

          <form action={createCrew} className="mt-8 grid gap-5">
            <label className="grid gap-2">
              <span className="font-bold text-white">Weekend Crew Person *</span>
              <select name="operationsContactId" required className={inputClass}>
